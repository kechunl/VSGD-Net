...
def fastrcnn_loss(class_logits, box_regression, labels, regression_targets, use_focal, focal_loss_gamma):
    # type: (Tensor, Tensor, List[Tensor], List[Tensor], Bool, Float) -> Tuple[Tensor, Tensor]
    """
    ...
    if use_focal:
        if torch.max(labels) > 1:
            # focal loss only be calculated in binary classification
            raise
        classification_loss = sigmoid_focal_loss(class_logits, F.one_hot(labels).float(), gamma=focal_loss_gamma, reduction='mean')
    else:
...
class RoIHeads(nn.Module):
    ...
    def __init__(
        ...
        self.use_focal = (use_focal > 0)
        self.focal_loss_gamma = use_focal if self.use_focal else 0
    ...
    def forward(
        ...
        if self.training:
            ...
            loss_classifier, loss_box_reg = fastrcnn_loss(class_logits, box_regression, labels, regression_targets, self.use_focal, self.focal_loss_gamma)
